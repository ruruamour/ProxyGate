...
| 模式 | 环境变量 | 说明 |
|------|---------|------|
| 黑名单 | `BLOCKED_COUNTRIES` | 屏蔽指定国家，其余放行（程序本地默认空；`.env.example`/docker-compose 示例为 `CN`） |
...
1. **示例配置**：如果沿用 `.env.example` 或 `docker-compose.yml` 示例，保持 `BLOCKED_COUNTRIES=CN` 适合大多数海外出口场景
